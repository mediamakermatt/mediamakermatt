...
var requiredModules = ["PathfindingLibrary"];
...
var cellSize = 12;
var range_buffer_value = 50;
if(character.range < range_buffer_value){
    var playerBuffer = character.range * .5;
    var monsterBuffer = character.range * .5;
} else {
    var playerBuffer = range_buffer_value - 5;
    var monsterBuffer = range_buffer_value - 5;
}
var farmTypes = JSON.parse(localStorage.getItem("farmTypes"));
var potions = JSON.parse(localStorage.getItem("potionTypes"));
//var monsterBlacklist = populateMonsterBlacklist();
var avoidTypes = farmTypes; // find_dangerous_monsters_on_destination_map();
var avoidRage = ["mummy"];
var gridCache = {};
...
var pathIndex = 1;
var lastMove;
var minTimeBetweenMovements = character.ping;
...
function disablePlayerCells(grid, map) {
    ...
    for (id in players) {
        ...
        if(distToPlayer <= playerRange || true){
...
function disableMonsterCells(grid, map) {
    ...
    for (id in monsters) {
        ...
        if(avoidTypes.includes(monster.mtype)){
            if(parent.G.monsters[monster.mtype].range + monsterBuffer < (character.range * .9)){
                var monsterRange = parent.G.monsters[monster.mtype].range + monsterBuffer;
            } else {
                if(parent.G.monsters[monster.mtype].range < (character.range * .9)){
                    var monsterRange = parent.G.monsters[monster.mtype].range;
                } else {
                    var monsterRange = (character.range * .9);
                }
            }
...
function populateMonsterBlacklist(){
    var monsterBlacklist = [];
    var health = parent.G.items[potions[0]].gives[0][1];
    for(let i in parent.G.monsters){
        let monster = parent.G.monsters[i];
        if(monster){
            let monsterDmgSecond = (monster.frequency * monster.attack) * (G.skills.use_hp.cooldown / 1000);
            if(health <= monsterDmgSecond){
                monsterBlacklist.push(monster.skin)
            }
        }
    }
    return monsterBlacklist;
}
    ...
    for(world_object in world_map_objects){
        ...
        if(map){
            for(monster_object in map.map.monsters){
                ...
                if(farmTypes.includes(monster.type)){
                    let data = {
                        map: map.name,
                        monster: monster
                    };
...
function find_dangerous_monsters_on_destination_map(){
    var mapName = JSON.parse(localStorage.getItem('destinationMap'));
    var mapMonsters = parent.G.maps[mapName].monsters;
    var types = [];
    for(let i in mapMonsters){
        var mType = mapMonsters[i].type;
        if([...new Set(farmTypes.concat(monsterBlacklist))].includes(mType)){
            types.push(mType);
        }
    }
    var unique = [...new Set(types)];
    var safeAmt = [unique[0]];
    if(unique.length > safeAmt.length){
        return safeAmt;
    }
    return unique;
}